...
// ────────────────────────────────────────────────
//  CONFIG (your project – cloudchat-bcbad)
// ────────────────────────────────────────────────
  ...
  if (user) {
    ...
    if (!user.displayName) {
      try {
        ...
        console.warn("Could not set initial displayName", err);
      }
    }
...
document.getElementById('saveNameBtn').onclick = async () => {
  ...
  try {
    ...
    errorEl.textContent = 'Failed to save – check connection';
...
document.getElementById('sendBtn').onclick = async () => {
  ...
  try {
    ...
    showStatus("Failed to send – check connection", '#ef4444');
